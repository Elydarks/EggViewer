	...
	private void onItemTooltip(ItemStack stack, TooltipContext context, List<Text> lines) {
		if (stack.getItem() == Items.TURTLE_EGG) {
			...
			if (nbt != null) {
				...
				lines.add(Text.literal("Form: ").setStyle(Style.EMPTY.withColor(Formatting.DARK_PURPLE))
						.append(Text.literal(Form)).append(Text.literal(genderSymbol).setStyle(genderColor)
				...
				if (!eggMoves.isEmpty()) {
					String eggmovesString = "[ \n";
					for (int i= 0; i < eggMoves.size(); i++) {
						NbtCompound moveTag = eggMoves.getCompound(i);
						eggmovesString += "  " + moveTag.getString("MoveName");
						if (i < eggMoves.size() - 1) {
							eggmovesString += ", \n";
						}
					}
					eggmovesString += "\n ]";
					lines.add(Text.literal("eggmoves: ").setStyle(Style.EMPTY.withColor(Formatting.DARK_PURPLE)).append(Text.literal(eggmovesString).setStyle(Style.EMPTY.withColor(Formatting.WHITE))));
				}
